...
const SectionHeader = ({ icon: Icon, label }) => (
  <div className="flex items-center gap-2 mb-4">
    <Icon size={13} className="text-cream/40 flex-shrink-0" />
...
export default function DashboardPage() {
  ...
  // Auto-refresh every 10s while any recent session is still active (ended_at not set).
  // Only watches sessions created in the last 30 minutes to avoid looping on orphaned old records.
  useEffect(() => {
  ...
  // Avg dimension scores across all evaluated sessions
  const dimAvg = (() => {
  ...
  return (
    <main className="min-h-screen bg-[#0c1119] pb-28">
      <div className="mx-auto max-w-[1320px] px-6 py-10 sm:px-10">

        {/* ── Page header ──────────────────────────────────────────────────── */}
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            <Link to="/app" className="inline-flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-cream/40 hover:text-neon transition-colors mb-4">
              <ArrowLeft size={12} /> Back to App
            </Link>
            <div className="flex items-center gap-3 mb-2">
              <span className="inline-flex items-center gap-2 rounded-full border border-neon/20 bg-neon/5 px-4 py-1.5 text-[10px] uppercase tracking-[0.3em] text-neon">
                <Sparkles size={11} /> Recruitment Command Centre
              </span>
            </div>
            <h1 className="text-[42px] font-black uppercase tracking-[-0.04em] text-cream leading-none">
              Candidate Pipeline<br />
              <span className="text-neon">Intelligence</span>
            </h1>
          </div>
          <button onClick={() => fetchSessions(true)} disabled={refreshing}
            className="mt-2 inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-5 py-2.5 text-[11px] font-black uppercase tracking-widest text-cream/50 hover:border-neon/30 hover:text-neon transition-all disabled:opacity-40">
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-3 lg:grid-cols-6">
          {[
            ...
            <motion.div key={label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              className="rounded-[24px] border border-white/5 bg-white/5 p-5">
              ...
              <p className="text-[10px] text-cream/30 mt-1 uppercase tracking-wider">{sub}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-[24px] border border-white/5 bg-white/5 px-6 py-5 flex items-center gap-4">
            <Award size={28} className="text-amber-400 flex-shrink-0" />
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40">Avg Intent Score</p>
              <p className={`text-3xl font-black mt-1 ${scoreColor(parseFloat(avgScore))}`}>
                {avgScore ?? '—'}<span className="text-sm text-cream/30 ml-1">/10</span>
              </p>
            </div>
          </div>
          <div className="rounded-[24px] border border-white/5 bg-white/5 px-6 py-5 flex items-center gap-4">
            <Activity size={28} className="text-violet-400 flex-shrink-0" />
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40">Eval Confidence</p>
          ...
          <div className="rounded-[24px] border border-white/5 bg-white/5 px-6 py-5 flex items-center gap-4">
            <TrendingUp size={28} className="text-sky-400 flex-shrink-0" />
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40">Answer Rate</p>
              <p className="text-3xl font-black text-sky-400 mt-1">
                {total ? `${pct(answered, total)}%` : '—'}
              </p>
            </div>
          </div>
          <div className="rounded-[24px] border border-white/5 bg-white/5 px-6 py-5 flex items-center gap-4">
            <Layers size={28} className="text-emerald-400 flex-shrink-0" />
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40">Conversion</p>
              <p className="text-3xl font-black text-emerald-400 mt-1">
                {answered ? `${pct(interested, answered)}%` : '—'}
              </p>
              <p className="text-[10px] text-cream/30 mt-0.5">answered → interested</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8 grid gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-[28px] border border-white/5 bg-white/5 p-6">
            ...
            {Object.keys(outcomeCounts).length === 0
              ? <p className="text-sm text-cream/30">No data yet</p>
          ...
          <div className="rounded-[28px] border border-white/5 bg-white/5 p-6">
            ...
            {topSkills.length === 0
              ? <p className="text-sm text-cream/30">No verified skills yet</p>
              : <div className="flex flex-wrap gap-2">
                  {topSkills.map(([skill, count]) => (
                    <span key={skill} className="inline-flex items-center gap-1.5 rounded-full border border-neon/20 bg-neon/5 px-3 py-1 text-[10px] font-black uppercase tracking-[0.15em] text-neon">
                      {skill}
        ...
        {evaluated > 0 && (
          <div className="mb-8 rounded-[28px] border border-white/5 bg-white/5 p-6">
                ...
                <thead>
                  <tr className="border-b border-white/10">
                    {['Rank','Candidate','Score','Outcome','Fit','Recommendation','Current CTC','Expected CTC','Notice','Engagement','Called'].map(h => (
                      <th key={h} className="pb-3 pr-4 text-left text-[9px] uppercase tracking-[0.35em] text-cream/30 font-normal whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[...scoredSes].sort((a, b) => b.intent_score - a.intent_score).slice(0, 8).map((s, i) => {
                    ...
                    return (
                      <tr key={s.id ?? i} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                        ...
                        <td className="py-3 pr-4">
                          <span className={`text-2xl font-black ${scoreColor(s.intent_score)}`}>{s.intent_score}</span>
                          <span className="text-cream/20 text-xs">/10</span>
                        </td>
                        <td className="py-3 pr-4">
                          <Pill style={outcomeStyle(s.call_outcome)} size="xs">{s.call_outcome || '—'}</Pill>
                        </td>
                        <td className="py-3 pr-4">
                          <Pill style={compatStyle(cs.compatibility_level)} size="xs">{cs.compatibility_level || '—'}</Pill>
                        </td>
                        <td className="py-3 pr-4">
                          <Pill style={recStyle(cs.recommendation)} size="xs">{cs.recommendation || '—'}</Pill>
                        </td>
                        <td className="py-3 pr-4 text-cream/60 text-xs">{n.current_ctc_lpa != null ? `${n.current_ctc_lpa} LPA` : '—'}</td>
                        <td className="py-3 pr-4 text-cream/60 text-xs">{n.salary_expectation_lpa != null ? `${n.salary_expectation_lpa} LPA` : '—'}</td>
                        <td className="py-3 pr-4 text-cream/60 text-xs">{n.notice_period_days != null ? `${n.notice_period_days}d` : '—'}</td>
                        <td className="py-3 pr-4">
                        ...
                        <td className="py-3 text-cream/30 text-xs whitespace-nowrap">{fmt(s.created_at)}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <div className="space-y-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.35em] text-cream/40">Full call log</p>
              <h2 className="mt-1 text-2xl font-black text-cream">All Candidate Sessions</h2>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select value={sortBy} onChange={e => setSortBy(e.target.value)}
                className="rounded-2xl border border-white/10 bg-[#0c1119] px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-cream/50 outline-none cursor-pointer">
          ...
          {loading && (
            <div className="rounded-[28px] border border-white/10 bg-slate-950/80 p-14 text-center text-cream/40">
              Loading sessions…
            </div>
          )}
          {error && (
...
function CandidateCard({ session, isOpen, onToggle }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      className="overflow-hidden rounded-[28px] border border-white/5 bg-white/5">

      {/* ── Collapsed header ─────────────────────────────────────────────── */}
      <button type="button" onClick={onToggle}
        className="w-full px-7 py-5 text-left hover:bg-white/[0.03] transition-colors">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">

          {/* Left: Identity + meta */}
          <div className="flex items-start gap-5">
            <div className="flex-shrink-0 w-12 h-12 rounded-2xl bg-neon/10 border border-neon/20 flex items-center justify-center text-neon font-black text-lg uppercase">
              {(session.candidate_name || 'C')[0]}
            </div>
            <div className="space-y-2 min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-lg font-bold text-cream uppercase tracking-tight">
                ...
                {n.engagement_level && (
                  <Pill style={engStyle(n.engagement_level)}>{n.engagement_level} engagement</Pill>
                )}
              </div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-cream/35">
              ...
              {/* Vibe check in collapsed if evaluated */}
              {n.vibe_check && (
                <p className="text-[11px] text-cream/40 italic leading-5 max-w-xl">"{n.vibe_check}"</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 flex-shrink-0">
            {/* Score */}
            <div className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-center min-w-[60px]">
              <p className="text-[9px] uppercase tracking-[0.3em] text-cream/30">Score</p>
              <p className={`text-2xl font-black mt-0.5 ${scoreColor(session.intent_score)}`}>{session.intent_score ?? '—'}</p>
            </div>
            {/* Outcome */}
            {session.call_outcome && (
              <div className={`rounded-2xl border px-4 py-3 text-center ${outcomeStyle(session.call_outcome).bg} ${outcomeStyle(session.call_outcome).border}`}>
                <p className="text-[9px] uppercase tracking-[0.3em] text-cream/30">Outcome</p>
            ...
            {cs.compatibility_level && (
              <div className={`rounded-2xl border px-4 py-3 text-center ${compatStyle(cs.compatibility_level).bg} ${compatStyle(cs.compatibility_level).border}`}>
                <p className="text-[9px] uppercase tracking-[0.3em] text-cream/30">Fit</p>
            ...
            {n.notice_period_days != null && (
              <div className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-center">
                <p className="text-[9px] uppercase tracking-[0.3em] text-cream/30">Notice</p>
            ...
            {n.salary_expectation_lpa != null && (
              <div className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-center">
                <p className="text-[9px] uppercase tracking-[0.3em] text-cream/30">Exp. CTC</p>
            ...
            {(n.hr_flags || []).length > 0 && (
              <div className="rounded-2xl border border-red-400/20 bg-red-400/5 px-4 py-3 text-center">
                <p className="text-[9px] uppercase tracking-[0.3em] text-red-400/60">Flags</p>
                <p className="text-[11px] font-black text-red-400 mt-0.5">{n.hr_flags.length}</p>
              </div>
            )}
            <div className="rounded-full border border-white/10 bg-slate-950/50 p-2.5 text-cream/30">
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div key="body"
            ...
            <div className="border-t border-white/10 px-7 py-7 space-y-8">
              ...
              <Section icon={Target} label="Executive Decision">
                ...
                {n.recommended_next_step && (
                  <div className="mt-4 flex items-start gap-3 rounded-2xl border border-neon/15 bg-neon/5 px-5 py-4">
                    <ArrowRight size={14} className="text-neon flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="text-[10px] uppercase tracking-[0.35em] text-neon/60 mb-1">Recommended Next Step</p>
                      <p className="text-sm text-cream/80 leading-6">{n.recommended_next_step}</p>
                    </div>
                  </div>
                )}
                {cs.recommendation && (
                  <div className="mt-3 flex items-start gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-4">
                    <Info size={14} className="text-cream/30 flex-shrink-0 mt-0.5" />
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-[10px] uppercase tracking-[0.35em] text-cream/40">Recommendation</p>
                        <Pill style={recStyle(cs.recommendation)}>{cs.recommendation}</Pill>
                      </div>
                      <p className="text-sm text-cream/60 leading-6">{cs.recommendation_reason || '—'}</p>
                    </div>
                  </div>
                )}
              </Section>
              ...
              <Section icon={DollarSign} label="Logistics & Availability">
                <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-5">
                  ...
                  <DataPoint label="Expected CTC" value={n.salary_expectation_lpa != null ? `${n.salary_expectation_lpa} LPA` : null}
                    accent={n.salary_expectation_lpa != null && ic.ctc_range
                      ? 'text-cream' : undefined} />
                ...
                <Section icon={CheckCircle2} label={`Screening Checkpoints — ${n.checkpoints_completed.length}/8 Covered`}>
                  <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
                    {CHECKPOINTS.map(cp => {
                      ...
                      return (
                        <div key={cp.key} className={`flex items-center gap-3 rounded-2xl border px-4 py-3 transition-colors ${done ? 'border-emerald-400/20 bg-emerald-400/5' : 'border-white/5 bg-white/[0.02]'}`}>
                          {done
                            ? <CheckCircle2 size={14} className="text-emerald-400 flex-shrink-0" />
                            : <XCircle size={14} className="text-white/15 flex-shrink-0" />
                          }
                    ...
                    {[
                      ...
                      <div key={key} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                        <ScoreBar
                          score={ds[key].score}
                          label={label}
                          confidence={ds[key].confidence}
                          evidence={ds[key].evidence}
                        />
                      </div>
                    ))}
                  </div>
                ...
                <Section icon={Activity} label="Behavioural Signals & Engagement">
                  <div className="grid gap-5 sm:grid-cols-3">
                    {n.interest_indicators?.length > 0 && (
                      <div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/5 p-5">
                        <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-emerald-400/70 mb-3">
                    ...
                    {n.concern_indicators?.length > 0 && (
                      <div className="rounded-2xl border border-red-400/15 bg-red-400/5 p-5">
                        <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-red-400/70 mb-3">
                    ...
                    {n.tone_signals?.length > 0 && (
                      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                        <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-cream/40 mb-3">
                          <Eye size={11} /> Tone Observations
                        </p>
                        <ul className="space-y-2">
                          {n.tone_signals.map((x, i) => (
                            <li key={i} className="flex items-start gap-2 text-xs text-cream/60 leading-5">
                              <span className="mt-1.5 w-1 h-1 rounded-full bg-white/30 flex-shrink-0" />{x}
                            </li>
                          ))}
                        </ul>
                      </div>
                  ...
                  {n.engagement_level && (
                    <div className="mt-4 flex items-center gap-3">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40">Overall Engagement:</p>
              ...
              {(cs.match_points?.length > 0 || cs.gap_points?.length > 0 || cs.missing_skills?.length > 0 || cs.red_flags?.length > 0) && (
                <Section icon={Layers} label="Role Fit Analysis">
                  <div className="grid gap-5 sm:grid-cols-2">
                      ...
                      <div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/5 p-5">
                        ...
                        <ul className="space-y-2">
                          {cs.match_points.map((x, i) => (
                            <li key={i} className="flex items-start gap-2 text-xs text-cream/65 leading-5">
                    ...
                    {cs.gap_points?.length > 0 && (
                      <div className="rounded-2xl border border-amber-400/15 bg-amber-400/5 p-5">
                        <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-amber-400/70 mb-3">
                  ...
                  {cs.missing_skills?.length > 0 && (
                    <div className="mt-4">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40 mb-2">Missing Required Skills</p>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {cs.red_flags?.length > 0 && (
                    <div className="mt-4 space-y-2">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-red-400/60 flex items-center gap-2"><Shield size={10} /> Hard Blockers</p>
                  ...
                  {cs.compatibility_reason && (
                    <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3">
                      <p className="text-[10px] uppercase tracking-[0.3em] text-cream/30 mb-1">Compatibility Verdict</p>
                      <p className="text-sm text-cream/70 leading-6">{cs.compatibility_reason}</p>
                    </div>
                  )}
                </Section>
              )}
              ...
              <Section icon={Shield} label="HR Notes & Verified Skills">
                <div className="grid gap-5 sm:grid-cols-2">
                  <div>
                    {(n.hr_flags || []).length > 0 ? (
                      <div className="rounded-2xl border border-red-400/15 bg-red-400/5 p-5">
                        <p className="text-[10px] uppercase tracking-[0.3em] text-red-400/60 mb-3 flex items-center gap-2">
                          <AlertTriangle size={11} /> HR Flags — Action Required
                        </p>
                        <ul className="space-y-2">
                          {n.hr_flags.map((f, i) => (
                            <li key={i} className="flex items-start gap-2 text-xs text-red-200/80 leading-5">
                      ...
                      <div className="rounded-2xl border border-emerald-400/10 bg-emerald-400/5 px-5 py-4 flex items-center gap-3">
                  ...
                  <div>
                    {(n.skills_verified || []).length > 0 ? (
                      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                        <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40 mb-3 flex items-center gap-2">
                          <BadgeCheck size={11} /> Skills Confirmed on Call
                        </p>
                        <div className="flex flex-wrap gap-2">
                          {n.skills_verified.map(sk => (
                            <span key={sk} className="rounded-full border border-neon/20 bg-neon/5 px-3 py-1 text-[10px] font-black uppercase tracking-[0.15em] text-neon">{sk}</span>
                          ))}
                        </div>
                      </div>
                    ) : (
                      <div className="rounded-2xl border border-white/10 bg-white/[0.02] px-5 py-4">
                        <p className="text-sm text-cream/30">No skills verified on this call</p>
                      </div>
                    )}
                  </div>
                </div>
                {n.vibe_check && (
                  <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.02] px-5 py-4">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-cream/30 mb-1.5">Vibe Check</p>
                    <p className="text-sm text-cream/70 leading-6 italic">"{n.vibe_check}"</p>
                  </div>
                )}
              </Section>

              {/* 8 ── Summary Bullets & Reasoning ────────────────────────── */}
              <Section icon={Zap} label="AI Evaluation Reasoning">
                {(n.summary_bullets || cs.summary_bullets || []).length > 0 && (
                  <div className="mb-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40 mb-3">Key Observations</p>
                    <ul className="space-y-2">
                      {(n.summary_bullets?.length ? n.summary_bullets : cs.summary_bullets || []).map((b, i) => (
                        <li key={i} className="flex items-start gap-2.5 text-sm text-cream/65 leading-6">
                ...
                {(session.eval_reasoning || n.reasoning) && (
                  <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40 mb-3">Evaluator Reasoning</p>
                    <p className="text-sm text-cream/60 leading-7 whitespace-pre-wrap">
                      {session.eval_reasoning || n.reasoning}
                    </p>
                  </div>
                )}
                ...
                <Section icon={FileText} label="Role Context & Resume">
                  <div className="grid gap-5 sm:grid-cols-2">
                    {session.job_description && (
                      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                        <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40 mb-3">Job Description</p>
                        <p className="text-xs text-cream/50 leading-6 whitespace-pre-line">{clip(session.job_description, 800)}</p>
                      </div>
                    )}
                    {session.resume_text && (
                      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                        <p className="text-[10px] uppercase tracking-[0.3em] text-cream/40 mb-3">Candidate Resume</p>
                        <p className="text-xs text-cream/50 leading-6 whitespace-pre-line">{clip(session.resume_text, 800)}</p>
                      </div>
                    )}
                  </div>
                </Section>
              ...
              {(session.transcript || []).length > 0 && (
                <Section icon={MessageSquare} label={`Full Call Transcript — ${session.transcript.length} Messages`}>
                  <div className="max-h-[600px] overflow-y-auto space-y-3 pr-1 rounded-2xl border border-white/10 bg-black/20 p-5">
                    {session.transcript.map((msg, i) => {
                      const isAI = msg.role === 'assistant';
                      return (
                        <div key={i} className={`flex gap-3 ${isAI ? '' : 'flex-row-reverse'}`}>
                          <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black uppercase ${isAI ? 'bg-neon/20 text-neon border border-neon/30' : 'bg-white/10 text-cream/60 border border-white/10'}`}>
                            {isAI ? 'AI' : (session.candidate_name?.[0] || 'C')}
                          </div>
                          <div className={`max-w-[78%] rounded-2xl px-4 py-3 ${isAI ? 'bg-neon/5 border border-neon/10 rounded-tl-sm' : 'bg-white/5 border border-white/10 rounded-tr-sm'}`}>
                            <p className={`text-[9px] font-black uppercase tracking-[0.25em] mb-1 ${isAI ? 'text-neon/50' : 'text-cream/25'}`}>
                              {isAI ? 'Priya AI' : (session.candidate_name || 'Candidate')}
                            </p>
                            <p className="text-sm text-cream/70 leading-6">{msg.content}</p>
                          </div>
                        </div>
                      );
                    })}
    ...
    <div>
      <div className="flex items-center gap-2.5 mb-4 pb-3 border-b border-white/5">
        <div className="w-6 h-6 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center">
          <Icon size={12} className="text-cream/40" />
        </div>
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-cream/40">{label}</p>
      </div>
      {children}
